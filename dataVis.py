...
chosen_mineral = 'Gold'
chosen_mineral_index = elements.index(chosen_mineral)

chosen_counts = counts[:, chosen_mineral_index, :, :]
chosen_totals = np.sum(chosen_counts, axis=(1, 2))

# Filter squares with non-zero chosen mineral counts
non_zero_chosen_indices = np.where(chosen_totals > 0)[0]


for i, mineral_name in enumerate(elements):
    mineral_totals = np.sum(counts[:, i, :, :], axis=(1, 2))

    plt.figure(figsize=(10, 8))
    plt.scatter(chosen_totals[non_zero_chosen_indices], mineral_totals[non_zero_chosen_indices], alpha=0.4)

    # Fit a linear regression model
    X = chosen_totals[non_zero_chosen_indices].reshape(-1, 1)
    y = mineral_totals[non_zero_chosen_indices]
    model = LinearRegression().fit(X, y)
    y_pred = model.predict(X)
    r2 = r2_score(y, y_pred)

    # Plot the regression line
    plt.plot(X, y_pred, color='red', linewidth=2, label=f'Linear fit (R^2 = {r2:.2f})')

    plt.xlabel(f'{chosen_mineral} Count')
    plt.ylabel(f'{mineral_name} Count')
    plt.title(f'Scatter plot of {chosen_mineral} vs {mineral_name} Counts with Linear Fit\nR^2 = {r2:.2f}')
    plt.legend()
    plt.grid(True)
    plt.savefig(f"{pre_training_vis_path}/{chosen_mineral}_vs_{mineral_name}_scatter.png")
    plt.close()

print("Scatter plots with R² values saved in the 'preTrainingVis' folder.")
